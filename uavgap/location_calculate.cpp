...
std::string data_buf;//串口缓存区，存放16进制UWB数据
...
{
    ROS_INFO_STREAM("111");
    ros::init(argc,argv,"location_calculate_node");//初始化节点
    ros::NodeHandle nh;//声明节点句柄
    //发布消息
    //ros::Publisher dis_pub = nh.advertise<std_msgs::UInt32>("uwb_distance", 1);
	ros::Publisher location_pub = nh.advertise<geometry_msgs::Vector3>("uwb_position", 1);//uwb_position缓存区名称与大小
																						//location_pub用于发布消息
    //订阅PX4高度信息
	ros::Subscriber get_rel_alt_sub = nh.subscribe<std_msgs::Float64>
    ("/mavros/global_position/rel_alt", 1, get_rel_alt_cb);//get_rel_alt_cb回调函数名
...
ROS_INFO_STREAM("5");
    int data_length = 0;//存放串口缓存区中数据位数
    ...
    anchorArray[0].z = 1.340; //anchor0.z uint:m

    anchorArray[1].x = 10.000; //anchor1.x uint:m
    anchorArray[1].y = 0.000; //anchor1.y uint:m
    anchorArray[1].z = 1.800; //anchor1.z uint:m

    anchorArray[2].x = 10.000; //anchor2.x uint:m
    anchorArray[2].y = 10.000; //anchor2.y uint:m
    anchorArray[2].z = 1.520; //anchor2.z uint:m

    anchorArray[3].x = 0.000; //anchor3.x uint:m
    anchorArray[3].y = 10.000; //anchor3.y uint:m
    anchorArray[3].z = 1.540; //anchor3.z uint:m
	
    while (ros::ok())
    { 
ROS_INFO_STREAM("6");
        data_length = uwb.available();
ROS_INFO_STREAM("data: "<< data_length);
        if(data_length)
        { ROS_INFO_STREAM("77");
	       ...
	       ROS_INFO_STREAM("8");
               Range_deca[0] = numtrans(3); //tag to A0 distance
               Range_deca[1] = numtrans(15); //tag to A1 distance
	           Range_deca[2] = numtrans(27); //tag to A2 distance
	           Range_deca[3] = numtrans(39); //tag to A3 distance
		   ...
		   current_pos.z = current_rel_alt.data;//相对高度待测试，要是不行就直接用气压计
