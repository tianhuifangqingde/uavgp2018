  ...
  //(abs(global_pose.latitude-47.398317)<0.0003)&&(abs(global_pose.longitude-8.5460425)<0.0003)))
  {
    ...
    //cl.call(srv_setMode);
     ...
     if(get_target==1)
     {
       ...
       if(current_rel_alt.data>3.2)
       vs_body_axis.twist.linear.z = -0.15;
       else
        if(control_flag.data==1)
          vs_body_axis.twist.linear.z = 0.15;
	else
	  vs_body_axis.twist.linear.z = -0.10;
	...
	//ROS_INFO("control_flag::  %d",control_flag.data); 
     ...
     if((ros::Time::now() - last_request) > ros::Duration(40.0)){state=4;
