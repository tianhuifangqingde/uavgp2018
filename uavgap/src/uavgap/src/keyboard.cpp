...
int main(int argc, char **argv)
{
  ...
  struct target{float x;float y;float z;}target1,target2;
 ...
 //ROS_INFO("got current_rel_alt: %f",current_rel_alt.data);
  ...
  if((state==2)&&current_rel_alt.data>4.6)
  {
  ...
  //(abs(global_pose.latitude-47.398317)<0.0003)&&(abs(global_pose.longitude-8.5460425)<0.0003)))
  {
    ...
    // cl.call(srv_setMode);
     ...
     if(get_target==1)
     {
       ...
       vs_body_axis.twist.linear.z = -0.12;
  ...
  wp.z_alt        = 0;
  wp.x_lat        = target2.x;//39.962769;
  wp.y_long       = target2.y;//116.304855;
  srv_wp.request.waypoints.push_back(wp);
